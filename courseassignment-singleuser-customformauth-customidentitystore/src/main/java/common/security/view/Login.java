		...
		switch (continueAuthentication()) {
			...
			case SEND_FAILURE:
				...
				if (loginAttempts >= MAX_ATTEMPTS_ALLOWED) {
					...
					Faces.redirectPermanent("/errorpages/banned.html");
				}
				break;
			case SUCCESS:
				Faces.getFlash().setKeepMessages(true);
				Messages.addFlashGlobalInfo("Login succeed");
				Faces.redirect(Faces.getRequestContextPath() + "/course-assignments/index.xhtml");		// added for Caller-Initiated Authentication
				break;
			case NOT_DONE:
