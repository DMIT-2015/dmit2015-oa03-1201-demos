...
/**
 * Servlet implementation class LottoQuickPickServlet01
 */
@WebServlet("/servlet/LottoQuickPickServlet01")
public class LottoQuickPickServlet01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    LotteryCanada lotto = new LotteryCanada(LotteryCanada.LotteryType.LOTTO_MAX);
		...
		if (request.getContentType() == null) {
			request.setAttribute("quickPickArray", quickPickArray);
			request.getRequestDispatcher("/demo/servlet/lottoQuickPick01.jsp").forward(request, response);
		} else if (request.getContentType().equalsIgnoreCase("text/html")) {
