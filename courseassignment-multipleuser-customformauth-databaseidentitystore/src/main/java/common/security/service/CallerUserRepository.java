...
import common.security.interceptor.AdminOnlyPermissionSecurityInterceptor;
...
public class CallerUserRepository {
    ...
    public void add(CallerUser newCallerUser, String plainTextPassword, String[] groupNames) {
        ...
        em.persist(newCallerUser);
    }

    @Interceptors(AdminOnlyPermissionSecurityInterceptor.class)
    ...
    public Optional<CallerUser> findById(String id) {
        ...
        try {
            CallerUser querySingleResult = em.find(CallerUser.class, id);
    ...
    public List<CallerUser> list() {
        return em.createQuery("SELECT u FROM CallerUser u ORDER BY u.username", CallerUser.class).getResultList();
    }
